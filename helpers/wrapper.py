...
class Wrapper(WrapperAlgorithms):
    ...
    def get_training_loss(self, x):
        ...
        # Apply padding to the image tensor
        #padded_tensor = F.pad(x, (1, 1, 1, 1), mode='reflect')
        ...
        #gradient_x = F.conv2d(padded_tensor, prewitt_x.unsqueeze(0).unsqueeze(0))
        #gradient_y = F.conv2d(padded_tensor, prewitt_y.unsqueeze(0).unsqueeze(0))
        # Compute the gradient magnitude
        #edges = torch.sqrt(gradient_x.pow(2) + gradient_y.pow(2))
        #noise = torch.normal(0, (edges+0.01)/255)
        #noise = noise.cuda()
        #inp = x + noise
    ...
    def test(self, x, vis = False, map = False):
        ...
        noise = torch.cuda.FloatTensor(x.size()).normal_(mean=0, std=self.max_nrm_train_nl) #AG CG
        #noise = np.zeros((x.size()[2],x.size()[3]),dtype=np.uint8) #SP
        #cv2.randu(noise,0,255) #SP
        #noisew = cv2.threshold(noise,220,255,cv2.THRESH_BINARY)[1]*(-1) #SP
        #cv2.randu(noise,0,255) #SP
